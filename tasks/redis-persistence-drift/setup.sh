...
- Please also wire up Prometheus alerts on Redis persistence health
  (snapshot path + append-only log) so we get paged the next time durability
  regresses, instead of finding out from user complaints
